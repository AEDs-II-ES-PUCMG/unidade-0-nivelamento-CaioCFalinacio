...
public class Pedido {
	...
	private ItemDePedido[] itensPedido;
	...
	public Pedido(LocalDate dataPedido, int formaDePagamento) {
		...
		itensPedido = new ItemDePedido[MAX_PRODUTOS];
		quantProdutos = 0;
	...
	public boolean incluirProduto(ItemDePedido novo) {
		...
		if (quantProdutos < MAX_PRODUTOS) {
			itensPedido[quantProdutos++] = novo;
			return true;
		}
		return false;
	}
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			valorPedido += itensPedido[i].calcularSubtotal();
		}
	...
	public String toString() {
		...
		for (int i = 0; i < quantProdutos; i++ ) {
			stringPedido.append(itensPedido[i].toString() + "\n");
		}
	...
	public void mesclarPedidos(Pedido outroPedido){
		if(this.quantProdutos + outroPedido.quantProdutos <= MAX_PRODUTOS){
			for(int i = 0; i < outroPedido.quantProdutos; i++){
				for(int j = 0; j < quantProdutos; j++){
					if(this.itensPedido[j].equals(outroPedido.itensPedido[i])){
						int novaQuantidade = this.itensPedido[j].getQuantidade() + outroPedido.itensPedido[i].getQuantidade();
						this.itensPedido[j].setQuantidade(novaQuantidade);
						if(outroPedido.itensPedido[i].getPrecoVenda() < this.itensPedido[j].getPrecoVenda()){
							this.itensPedido[j].setPrecoVenda(outroPedido.itensPedido[i].getPrecoVenda());
						}
					}else{
						itensPedido[quantProdutos + 1] = outroPedido.itensPedido[i];
						quantProdutos++;
					}
				}
			}
			...
			throw new IllegalStateException("Quantidade máxima de produtos excedida!");
		}

	}

	public void imprimirRecibo(){
		StringBuilder stringRecibo = new StringBuilder();

		for(int i = 0; i < quantProdutos; i++){
			if(itensPedido[i] != null){
				stringRecibo.append("Nome do produto: " + itensPedido[i].getProduto().getDescricao() + "\n") ;
				stringRecibo.append("Quantidade: " + itensPedido[i].getQuantidade() + "\n") ;
				stringRecibo.append("Preço unitário: " + String.format("%.2f", itensPedido[i].getPrecoVenda()) + "\n") ;
				stringRecibo.append("Subtotal: " + String.format("%.2f", itensPedido[i].calcularSubtotal()) + "\n") ;
				stringRecibo.append("\n") ;
			}
		}
		stringRecibo.append("Preço total: R$ " + String.format("%.2f", valorFinal()) +"\n");
